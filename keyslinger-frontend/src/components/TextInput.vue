    ...
    checkCharacter: function (event) {
      ...
      if (this.active && !this.completed) {
        if (key === 'Backspace') {
          ...
          this.$store.dispatch('setTokenStatus', { index: this.currentIndex, status: '' })
        }
        // TODO: check special keys (enter, esq, shift, option)
        else if (key === this.tokens[this.currentIndex].key) {
          this.$store.dispatch('setTokenStatus', { index: this.currentIndex, status: 'ok' })
          this.$store.dispatch('incrementIndex')
        } else {
          this.$store.dispatch('setTokenStatus', { index: this.currentIndex, status: 'error' })
      ...
      if (this.currentIndex === this.tokens.length && !this.completed) {
        ...
        console.log('Done!') // TODO: show a message box
      }
    },
  },
